...
SSet :: struct($T: typeid) {
	// Sparse. Stores the idx for  entities list. It's idx is the Entity Ids.
	entity_idx: []int,

	// Dense. Stores the Entity Ids. Idx has no meaning, but must be valid idx stored in entity_idx.
...
resize_set :: proc(set: ^SSet($T)) {
    ...
    resize(&set.entities, new_capacity)
    resize(&set.components, new_capacity)
